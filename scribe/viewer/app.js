...
(function () {
  ...
  // Claude's mark at the foot of the column, as in the desktop app: still
  // when the conversation rests, turning while a turn is in progress. Working
  // means the board's card for this session says so (its phase is read off
  // the transcript's tail, refined by the daemon), or a driver of ours is
  // mid-turn.
  function isWorking() {
  ...
  // Anthropic's Claude mark, the path as published by Simple Icons.
  ...
  function syncWorking() {
    var column = $("column");
    var node = $("working");
    var show = state.view === "session" && state.rounds.some(Boolean);
    ...
    node.setAttribute("aria-label", busy ? "Claude is working" : "Claude");
